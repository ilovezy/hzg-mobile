<template>
  <div>
    <div ref="dragBox">
      <slot></slot>
      <div class="pullrefresh-dragtip" ref="dragTip" v-show="touches.isDraging" :class="dragTip.animationTiming" :style="{ 'transform': 'translate3d(0, ' + dragTip.translate + 'px, 0) scale(' + dragTip.scale + ')' }">
        <span :class="dragTip.loadingIcon" :style="{ 'transform': 'rotate(' + dragTip.iconRotate + 'deg)', opacity: dragTip.iconOpacity }"></span>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      init() {
        ...
        this.bindEvents();
        this.$on('cf.pullrefresh.finishLoad', this.finishLoad);
        this.showHelp();
      },
      showHelp() {
...
<style lang="less">
  @import "../../../assets/styles/components/pullrefresh.less";
</style>
