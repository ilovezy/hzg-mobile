  ...
  export default {
    ...
    methods: {
      ...
      checkParameters() {
        ...
        const min = ~~this.min;
        if (max < unit && max != 0) {
          console.error('[warn]: The parameter \'max\'(' + max + ') must be greater than or equal to \'unit\'(' + unit + ').');
          return false;
        }
        if (max % unit != 0) {
          console.error('[warn]: The parameter \'max\'(' + max + ') and \'unit\'(' + unit + ') must be multiple.');
          return false;
        }
        if (min % unit != 0 && min >= 0) {
          console.error('[warn]: The parameter \'min\'(' + min + ') and \'unit\'(' + unit + ') must be multiple.');
          return false;
        }
        if (max < min && max != 0) {
          console.error('[warn]: The parameter \'max\'(' + max + ') must be greater than to \'min\'(' + min + ').');
          return false;
        }
        return true;
      },
...
<style lang="less">
  @import "../../../assets/styles/components/spinner.less";
</style>
