<template>
  <div class="m-tab">
    <ul class="tab-nav">
      <li class="tab-nav-item" v-for="item in navList" :class="item._uid == activeIndex ? 'tab-active' : ''" @click="changeHandler(item._uid, item.label, item.tabkey)">
...
<script type="text/babel">
  export default {
    ...
    methods: {
      init(update) {
        const tabPanels = this.$children.filter(item => item.$options.name === 'tab-panel');
        let num = 0;
